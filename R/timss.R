get_catalog_timss <-
	function( data_name = "timss" , output_dir , ... ){
		...
		catalog <-
			data.frame(
				year = gsub( "(.*)([0-9][0-9][0-9][0-9])(.*)" , "\\2" , ftd ) ,
				full_url = ftd ,
				stringsAsFactors = FALSE
			)
			
		catalog$sas_url <-
			ifelse( catalog$year == 1995 ,
				ifelse( grepl( "POP1" , catalog$full_url ) , 'http://timss.bc.edu/timss1995i/database/pop1/POP1PGRM.ZIP' ,
				ifelse( grepl( "POP2" , catalog$full_url ) , 'http://timss.bc.edu/timss1995i/database/pop2/POP2PGRM.ZIP' ,
				ifelse( grepl( "POP3" , catalog$full_url ) , 'http://timss.bc.edu/timss1995i/database/pop3/POP3PGRM.ZIP' , NA ) ) ) ,
			ifelse( catalog$year == 1999 , 'http://timss.bc.edu/timss1999i/data/bm2_progs.zip' , 
				NA ) )
...
lodown_timss <-
	function( data_name = "timss" , catalog , ... ){
		...
		for ( i in seq_len( nrow( catalog ) ) ){
			...
			prefixes <- unique( substr( basename( unzipped_files ) , 1 , 3 ) )
			...
			if( catalog[ i , 'year' ] >= 2003 ){
				...
				for ( s in suf ){
					...
					for ( p in prefixes ){
						...
						cachaca( catalog[ i , 'sas_url' ] , tf , mode = 'wb' )
						
						sas_unzips <- unzip_warn_fail( tf , exdir = paste0( catalog[ i , 'output_folder' ] , '/unzips' ) )
					
						if( catalog[ i , 'year' ] == 1995 ){
						
							this.sas <- sas_unzips[ grep( toupper( paste0( p , "(.*)\\.sas" ) ) , toupper( basename( sas_unzips ) ) ) ]
						
							if( length( this.sas ) > 1 ) this.sas <- this.sas[ !grepl( "score" , tolower( this.sas ) ) ]
						
						} else {
						
							this.sas <- sas_unzips[ grep( toupper( paste0( p , "(.*)" , s , "\\.sas" ) ) , toupper( basename( sas_unzips ) ) ) ]
